...
export function getClassAttendanceData(id, callback) {
  let array1 = [];
  let array2 = [];
  let attendanceList = [];
  let attendanceSheet = [];
  ...
  con.connect(function (err) {
    ...
    con.query(
      mysql.format(command0 + command + command2),
      function (err, result, fields) {
        ...
        array1.forEach((item) =>
          array2.push(moment.utc(item).format("MM.DD.YY"))
        );
        let final = {
          name: classinfo[0],
          id: classinfo[1],
...
export function postNewAttendanceData(obj, callback) {
  console.log("news ", obj);
  let enrolled = [];
  let checked = obj.values;
  let names = [];
  checked.forEach((attendance) => {
    names.push(attendance.student);
  });
      ...
      con.connect(function (err) {
        ...
        enrolled.forEach((item) => {
          if (names.includes(item.fullName)) {
            var person = checked.find(
              (check) => check.student === item.fullName
            );
            command =
              command +
              `INSERT INTO attendance (value,date,classID,studentID,fullName) VALUES ('${person.value}','${obj.date}',${obj.class},${item.studentID},'${item.fullName}');`;
            console.log(command);
          } else {
            command =
              command +
              `INSERT INTO attendance (value,date,classID,studentID,fullName) VALUES ('${null}','${
                obj.date
              }',${obj.class},${item.studentID},'${item.fullName}');`;
            console.log(command);
          }
        });
        command.slice(0, -1);
...
export function postModifiedAttendanceData(obj, callback) {
  ...
  con.connect(function (err) {
    ...
    obj.changes.forEach((item) => {
      command =
        command +
        `UPDATE attendance SET value = '${item.value}' WHERE date = '${moment(
          item.name,
          "MM.DD.YY"
        ).format("YYYY-MM-DD[T]HH:mm:ss")}' AND fullName = '${
          item.student
        }' AND classID = ${obj.class};`;
    });
    command.slice(0, -1);
    var sql = mysql.format(command);
    con.query(sql, function (err, result, fields) {
...
function getEnrollments(classID, callback) {
  ...
  con.connect(function (err) {
    ...
    var sql = mysql.format(
      'SELECT CONCAT(student.name," ",student.surname) as fullName,classID,studentID FROM enrollment INNER JOIN student ON enrollment.studentID = student.ID WHERE classID = ?',
      [classID]
    );
    con.query(sql, function (err, result, fields) {
