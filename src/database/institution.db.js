...
export function getUserData(obj, callback) {
  ...
  con.connect(function (err) {
    if (err) throw err;
    var command = `SELECT ID as id,email,password,CONCAT(name," ",surname) as name,institutionID as institution FROM teachers WHERE email = '${obj.email}' ;`;
    var command2 = `SELECT admin.ID as id,email,password,institution.name as name,institutionID as institution FROM admin INNER JOIN institution ON institution.ID = admin.institutionID WHERE email = '${obj.email}' `;
...
export function postInstitutionData(obj, callback) {
  getInstitutionCount(function (err, data) {
    if (err) {
      ...
      con.connect(function (err) {
        ...
        con.query(
          mysql.format(command2 + command1),
          function (err, result, fields) {
            if (err) callback(err, "Objects not inserted!");
            else callback(null, "Objects inserted!");
          }
        );
        con.end();
      });
