...
export function getAllStudentsData(institution, callback) {
  let data = {};
  let final = {};
  ...
  con.connect(function (err) {
    ...
    var sql = mysql.format(
      "SELECT CONCAT(student.name,' ',student.surname) as name,student.ID as id FROM student WHERE institutionID = ?",
      [institution]
    );
...
export function getSpecificStudentData(id, callback) {
  /* let gradeList = [];
  let gradeSheet = [];
  let nameList = [];
  var attList = [];
  ...
  con.connect(function (err) {
    ...
    con.query(
      mysql.format(command0 + command1 + command2),
      function (err, result, fields) {
        ...
        if (info.length == 0) {
          ...
          grades.forEach(function (item) {
            if (nameList.includes(item.name) == false) nameList.push(item.name);
          });
          ...
          attendance.forEach(function (check) {
            if (dateList.includes([check.date, check.name]) == false)
              dateList.push([check.date, check.name]);
          });
...
export function postStudentData(institution, obj, callback) {
  getStudentCount(function (err, data) {
    if (err) {
      ...
      con.connect(function (err) {
        if (err) throw err;
        var sql = mysql.format(
          "INSERT INTO student (ID,name,surname,nr,age,institutionID) VALUES (?,?,?,?,?,?)",
          [counter, obj.name, obj.surname, obj.nr, obj.age, institution]
        );
        con.query(sql, function (err, result, fields) {
